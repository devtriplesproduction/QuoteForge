        ...
        <div>
          ...
          >
            <div className="doc-card doc-header">
              <div className="doc-pad">
                {/*
                  FIX 1: the black "INVOICE" pill used to sit flush against
                  the far-right edge of the header (justify-content came from
                  the stylesheet, likely space-between), leaving the logo
                  isolated at the start. Forcing flex-start here packs both
                  the logo and the badge together at the start of the
                  container, exactly as requested.
                */}
                <div className="doc-header__top" style={{ justifyContent: "flex-start", gap: "16px" }}>
                  <div className="doc-header__brand">
                    {/*
                      FIX 2 (root cause of the "Triple S [broken-icon] Production"
                      glitch): the logo <img> had no error handling, so when
                      /triplesimage.png fails to resolve the browser falls back
                      to rendering the `alt` text split around a broken-image
                      icon glyph — which is what actually looked like a
                      misaligned "INVOICE"/company name. Now it swaps to a
                      clean text/monogram fallback instead.
                    */}
                    {!logoFailed ? (
                      <img
                      ...
                      <div className="h-8 w-8 shrink-0 rounded bg-gray-900 flex items-center justify-center">
                        <span className="text-white text-xs font-bold">TS</span>
                      </div>
                    )}
                    <div className="min-w-0">
                      <div className="doc-company truncate">
                        {logoFailed ? "Triple S Production" : ""}
                      </div>
                      <div className="doc-meta truncate">{brandKit?.website || brandKit?.email || brandKit?.phone || ""}</div>
                    </div>
                  </div>
